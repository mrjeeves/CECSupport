...
//! automatically, waits for the user, or is off entirely. Ported verbatim from
//! `allmystuff-updater` (which took it from `myownmesh-updater`; it has no
//! engine coupling), so all three apps in the ecosystem share one set of
//! update semantics — a patch bump means the same thing everywhere.
